/**
 * One editor card: N identical sets of an exercise — same grouping model as the
 * web forms (src/features/routines/setGroups.ts there). The DB stays one row
 * per set; groups exist only in the UI. Weight is `number | null` in routine
 * templates, plain `number` in logged sessions — hence the type parameter.
 */
